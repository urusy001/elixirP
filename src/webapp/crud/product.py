...
async def create_product(db: AsyncSession, product: ProductCreate) -> Product:
    ...
    stmt = stmt.on_conflict_do_update(
        ...
        set_={
            ...
            "category_onec_id": stmt.excluded.category_onec_id,
        },
    )
...
async def get_product_with_features(db: AsyncSession, onec_id: str) -> Optional[Product]:
    """
    Fetch a product by its onec_id along with all its features.
    """
    result = await db.execute(
        select(Product)
        .options(selectinload(Product.features))  # eager-load features
