...
async def handle_deep_start(message: Message, command: CommandObject, state: FSMContext):
    ...
    if product_id:
        async with get_session() as session:
            ...
            x, y = str(product).split("\n\n\n", maxsplit=1)
            await message.answer_photo(photo=BufferedInputFile(file=bts, filename=f'{product_id}.png'), caption=x)
            await message.answer(y, reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Купить", web_app=WebAppInfo(url=f"{WEBAPP_BASE_DOMAIN}/product/{product_id}"))]]))
