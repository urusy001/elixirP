...
async def handle_inline_query(inline_query: InlineQuery, state: FSMContext):
    ...
    if data[0] == "search_user":
        ...
        if column_name == "id":
            ...
            else: result = InlineQueryResultArticle(id=uuid4(), title="Ошибка поиска", input_message_content=InputTextMessageContent(message_text="<b>Телеграм ID должен быть числом</b>"))
