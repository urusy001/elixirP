...
# 🔹 Set up logs/<logger_name>.txt file handler
...
async def check_completion(session, giveaway_id: int, tg_id: int, participant, message: Message) -> Participant:
    ...
    if participant and participant.is_completed and not getattr(participant, "participation_code", None):
        ...
        logger.info(
            "Assigned participation_code for user %s in giveaway %s | code=%s",
            tg_id,
            giveaway_id,
            code,
        )
...
async def handle_ref_start(message: Message, command: CommandStart, state: FSMContext):
    logger.info(
        "handle_ref_start | user_id=%s | args=%r",
    ...
    logger.debug(
        "Parsed referral for user %s: ref_id=%s, giveaway_id=%s",
        message.from_user.id,
        ref_id,
        giveaway_id,
    )
    ...
    async with get_session() as session:
        ...
        logger.info(
            "Created participant via referral | user_id=%s | giveaway_id=%s | ref_id=%s",
            message.from_user.id,
            giveaway_id,
            ref_id,
        )
...
async def handle_email(message: Message, state: FSMContext):
    ...
    logger.info(
        "Review check result for user %s | giveaway_id=%s | ok=%s",
        message.from_user.id,
        giveaway_id,
        result.get("ok", False),
    )
    ...
    if result.get("ok", False):
        ...
        async with get_session() as session:
            ...
            logger.info(
                "Updated participant with review for user %s | giveaway_id=%s | review_id=%s",
                message.from_user.id,
                giveaway_id,
                review_id,
            )
...
async def handle_order_code(message: Message, state: FSMContext):
    ...
    logger.debug(
        "Loaded giveaway and participant for order check | user_id=%s | giveaway_id=%s",
        message.from_user.id,
        giveaway_id,
    )
    ...
    logger.info(
        "Deal check result for user %s | giveaway_id=%s | order_code=%s | ok=%s",
        message.from_user.id,
        giveaway_id,
        order_code,
        result.get("ok", False),
...
async def handle_user_call(call: CallbackQuery, state: FSMContext, giveaway_bot):
    ...
    if data[0] == "main_menu":
        ...
        giveaway_id = int(data[1])
        logger.info(
            "User %s requested check_%s | giveaway_id=%s",
            user_id,
            what,
            giveaway_id,
        )
        ...
        if what == "subscription":
            ...
            logger.info(
                "Subscription check | user_id=%s | channel=@%s | result=%s",
                user_id,
                giveaway.channel_username,
                result,
            )
            if result:
            ...
            logger.info(
                "Refs check | user_id=%s | giveaway_id=%s | refs=%s | required=%s",
                user_id,
                giveaway_id,
                refs,
                giveaway.minimal_referral_amount,
            )
            ...
            logger.debug(
                "Switching to input mode %s for user %s | giveaway_id=%s",
                what,
                user_id,
                giveaway_id,
            )
